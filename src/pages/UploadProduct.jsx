...
export default function UploadProduct() {
    ...
    const [success, setScuccess] = useState(null);
    ...
    const vaildate = () => {
        ...
        if (!product.title.trim()) return "상품명을 입력해 주세요";
    ...
    const uploadSubmit = async (e) => {
        ...
        try {
            ...
            setScuccess('업로드가 정상적으로 완료 되었습니다.')
            setTimeout(() => {
                setScuccess(null)
            }, 3000)
            setFile(null)
        ...
        <Container>
            ...
            <FormWrapper onSubmit={uploadSubmit}>
                ...
                <input type="text" name="title" placeholder="상품명을 입력해주세요"
                    value={product.title} onChange={productInfoChange}
                />
                {/* 상품 가격 */}
                <input type="text" name="price" placeholder="상품 가격을 입력해주세요."
                    value={product.price} onChange={productInfoChange} />
                ...
                <input type="text" name="option"
                    ...
                    value={product.option} onChange={productInfoChange} />
                ...
                {/* 버튼의 submit이벤트는 form의 내용을 업로드 하는 이벤트이며, 
                onSubmit 실제 폼요소의 데이터를 넘기는 영역에서 정해줘야 한다. */}
                {success && (
                    <p>{success}</p>
                )}
                {error && (
